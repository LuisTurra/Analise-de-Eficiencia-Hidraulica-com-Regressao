...
# Carregar apenas sensores físicos
...
mse_linear, preds_linear = train_model(model_linear, train_dataloader, X_test_tensor, y_test)

model_ridge = LinearModel(input_size)
mse_ridge, preds_ridge = train_model(model_ridge, train_dataloader, X_test_tensor, y_test, l2_alpha=0.01)

model_lasso = LinearModel(input_size)
mse_lasso, preds_lasso = train_model(model_lasso, train_dataloader, X_test_tensor, y_test, l1_alpha=0.01)
...
plt.savefig('comparison_physical.png')
plt.show()
...
ax.grid(True)
ax.legend()
ax.set_title('Treinamento de Sensor Físico')
plt.savefig('3d_plot_physical.png')
plt.show()

# Salvar modelo e normalização
torch.save(best_model.state_dict(), 'modelofisico.pth')
np.save('X_min_physical.npy', X_min)
np.save('X_max_physical.npy', X_max)
print("Modelo físico salvo como 'modelofisico.pth'")
...
with open('training_results_fisico.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, indent=4, ensure_ascii=False)

print("Resultados salvos em 'training_results_fisico.json'")